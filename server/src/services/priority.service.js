...
// Deprecated: impact level is no longer used in the app.
...
const buildVocabulary = (samples) => {
  ...
  samples.forEach((sample) => {
    ...
    const words = sample.description
      .toLowerCase()
      .replace(/[^\w\s]/g, "")
      .split(/\s+/)
...
const extractTextFeatures = (description, vocabulary) => {
  ...
  const urgencyScore =
    words.length > 0 ? urgencyCount / Math.sqrt(words.length) : 0;
...
const encodeSample = ({ category, description }, encoder) => {
  ...
  const categoryIdx =
    (encoder.category.map[category] ?? 0) /
    Math.max(encoder.category.size - 1, 1);

  // Extract context from description
  const { textVector, urgencyScore, lengthScore } = extractTextFeatures(
    description || "",
    encoder.vocabulary
  );
...
export const initPriorityModel = async () => {
  ...
  const encodedSamples = samples.map((sample) =>
    encodeSample(sample, encoders)
  );
  const inputSize = encodedSamples[0].length;

  console.log(
    `Feature vector size: ${inputSize} (includes text context features)`
  );
    ...
    callbacks: {
      onEpochEnd: (epoch, logs) => {
        ...
        if (epoch % 25 === 0 || epoch === MODEL_CONFIG.epochs - 1) {
          console.log(
            `Epoch ${epoch}: loss=${logs.loss.toFixed(
              4
            )}, val_loss=${logs.val_loss.toFixed(4)}, mae=${logs.mae.toFixed(
              4
            )}`
          );
        }
        ...
        if (patienceCounter >= patience) {
          console.log(
            `Early stopping at epoch ${epoch}. Best val_loss: ${bestValLoss.toFixed(
              4
            )}`
          );
...
export const predictPriority = async (payload) => {
  try {
    ...
    console.log(
      `AI Prediction for "${payload.description?.substring(0, 50)}...":`,
      {
